...
const sortConversationsByPriority = (list) =>
  [...list].sort((a, b) => {
...
const normalizeConversations = (list) => {
  ...
  return sortConversationsByPriority(Array.from(byId.values()));
};
...
export const ImprovedSidebar = ({
  ...
  useEffect(() => {
    const loadConversations = () => {
      ...
      if (stored) {
        try {
          setConversations(normalizeConversations(JSON.parse(stored)));
        } catch (error) {
  ...
  const sortConversations = (list) => normalizeConversations(list);
